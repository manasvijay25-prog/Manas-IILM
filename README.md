...
**Netflix-GPT** is a high-performance streaming platform clone that integrates **React**, **Redux**, and **Gemini** to solve the classic _“What should I watch?”_ problem using **natural language search**.
...
LayerTechnology**Frontend**React.js (Hooks, React Router)**Styling**Tailwind CSS (Responsive Design)**State Management**Redux Toolkit**AI Integration**Gemini API **Data Source**TMDB (The Movie Database) API**Authentication**Firebase Authentication

✨ Key Features
--------------

*   **AI Search Bar**Search movies using natural language prompts powered by OpenAI GPT models.
...
*   **Multi-Language Support**Language switching for the GPT search interface_(English, Hindi, Spanish)_
...
2.  **AI Data Orchestration**
    ...
    *   The app sends the prompt to the **OpenAI API** to fetch recommended movie titles.
...
### 1\. AI Integration with Promise.all
...
`   const moviePromises = movieNames.map((movie) =>    fetchMovieDetailsFromTMDB(movie)  );  const movieResults = await Promise.all(moviePromises);   `
...
**Challenge:** Preventing crashes when API keys are missing in production.**Solution:**
...
**Challenge:** Excessive API calls for repeated searches.**Solution:**
...
`   VITE_TMDB_KEY=your_tmdb_key  VITE_GEMINI_API_KEY=your_openai_key   `
